...
typedef struct {
    ...
    float     wind_kmh;
    float     wind_mph_2min;     // 2-minute sustained mean (ASOS-style)
    float     wind_kmh_2min;
    float     gust_mph;          // peak 3-sec running mean in last 10 min (WMO)
    ...
    uint16_t  window_seconds;    // how full the 10-min ring is (0..600);
                                 // < 600 means the gust/avg numbers are still warming up
